...
def make_update_s1(scope, reuse = None, xi_1=0.999, xi_2=0.9):
    with tf.variable_scope(scope, reuse=reuse):
        transfer = []
        p_func_vars = U.scope_vars(U.absolute_scope_name("p_func"))
        target_p_func_vars = U.scope_vars(U.absolute_scope_name("target_p_func"))
        ...
        for i in range(len(vars)):
            para = tf.cast(tf.size(vars[i]), tf.float32)
            delta =  vars[i] - target_vars[i]
            m_op.append(m[i].assign(xi_1 * m[i] + (1 - xi_1) * delta))
            v_op.append(v[i].assign(xi_2 * v[i] + (1 - xi_2) * delta ** 2))
...
def make_update_s2(scope, reuse = None, xi_1=0.999, xi_2=0.9):
    with tf.variable_scope(scope, reuse=reuse):
        transfer = []
        p_func_vars = U.scope_vars(U.absolute_scope_name("p_func"))
        target_p_func_vars = U.scope_vars(U.absolute_scope_name("target_p_func"))
...
def make_update_s3(scope, reuse = None, xi_1=0.999, xi_2=0.9):
    with tf.variable_scope(scope, reuse=reuse):
        transfer = []
        p_func_vars = U.scope_vars(U.absolute_scope_name("p_func"))
        target_p_func_vars = U.scope_vars(U.absolute_scope_name("target_p_func"))
...
def critics_train(make_obs_ph, act_space, q_func, optimizer, grad_norm_clipping=None, scope="trainer", reuse=None, num_units=64):
    with tf.variable_scope(scope, reuse=reuse):
...
def actors_train(make_obs_ph, act_space, p_func, q_func, optimizer, grad_norm_clipping=None, scope="trainer", reuse=None, num_units=64):
    with tf.variable_scope(scope, reuse=reuse):
        # create distribtuions
        act_pdtype = make_pdtype(act_space)
...
class DDPGAgentTrainer(AgentTrainer):
    def __init__(self, name, p_model, q_model, obs_shape, act_space, args):
        self.name = name
        self.args = args
        self.n = 1
        ...
        if self.soft_update == 'adaptive1':
            self.m_op, self.v_op, self.op, self.tau_next, self.polyak, self.t_input, self.delta = make_update_s1(scope=self.name, xi_1=args.xi_1, xi_2=args.xi_2)
        if self.soft_update == 'adaptive2':
            self.m_op, self.v_op, self.op, self.tau_next, self.polyak, self.t_input, self.delta = make_update_s2(scope=self.name, xi_1=args.xi_1, xi_2=args.xi_2)
        if self.soft_update == 'adaptive3':
            self.m_op, self.v_op, self.op, self.tau_next, self.polyak, self.t_input, self.delta = make_update_s3(scope=self.name, xi_1=args.xi_1, xi_2=args.xi_2)
