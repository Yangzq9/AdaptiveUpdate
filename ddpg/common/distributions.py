...
from ddpg.common.multi_discrete import MultiDiscrete
...
class SoftCategoricalPd(Pd):
    ...
    def sample(self):
        ...
        # return tf.nn.tanh(self.logits - tf.log(-tf.log(u)))  #   -tf.log(-tf.log(u))
        length = self.logits.get_shape().as_list()[1]
        split_array = []
        for i in range(int(length/3)):
            split_array.append(3)
        out = tf.split(self.logits, num_or_size_splits=split_array, axis=1)
        for i in range(len(out)):
            # u = tf.random_uniform(tf.shape(out[i]))
            # u = 0.15 * (1 - out[i]) * 0.04 + 1 * 0.2 * tf.random_normal(tf.shape(out[i]))     # OU NOISE
            out[i] = U.softmax(out[i], axis=-1)
