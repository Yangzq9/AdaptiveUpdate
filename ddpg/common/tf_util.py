...
def load_state(fname, num = 0, num_adv = 0, num_ag = 0, saver=None):    # num=0:load normally   num=2:load without variables with "not fixed" name
    # num=0: commonly default   num=1: reserve   num=2: not load the mismatched critic variables
    ...
    if num == 1:
        ...
        numlist_down = []
        numlist_up = []
        for i in range(num_ag+num_adv):
            numlist_down.append(str(i))
            numlist_up.append(str(i+num_ag+num_adv))
        variables_to_restore = [v for v in variables if v.name.split('/')[0].split('_')[1] in numlist_down]
        saver1 = tf.train.Saver(variables_to_restore)
        saver1.restore(get_session(), fname1)

        fname2 = fname + 'ag/'
        variable_map = {}
        variables_to_restore = [v for v in variables if v.name.split('/')[0].split('_')[1] in numlist_up]
        for i, variable in enumerate(variables_to_restore):
            temp = variable.name.replace(':0', '').split('/')
            for j in range(num_ag+num_adv):
                temp[0] = temp[0].replace(numlist_up[j], numlist_down[j])
                temp[1] = temp[1].replace(numlist_up[j], numlist_down[j])
            temp_str = ''
            for j in range(len(temp)):
                if j == (len(temp) - 1):
                    ...
                    temp_str = temp_str + temp[j] + '/'
            variable_map[temp_str] = variable
        saver2 = tf.train.Saver(var_list=variable_map, reshape=True)
        saver2.restore(get_session(), fname2)
        return saver1, saver2
    if num == 2:
        variables = tf.global_variables()
        var_name_list = []
        for i, variable in enumerate(variables):
            if 'not_fixed' in variable.name:
                print('found one not-fixed variable, ignore it')
            else:
                var_name_list.append(variable)
        saver = tf.train.Saver(var_list=var_name_list, reshape=True)
        saver.restore(get_session(), fname)
        return saver

def load_estimator(fname, num = 0, num_adv = 0, num_ag = 0, saver=None):
    """Load all the variables to the current session from the location <fname>"""
