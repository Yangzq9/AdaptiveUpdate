...
env_config = [['InvertedDoublePendulumBulletEnv-v0', 100, 500], ['HopperBulletEnv-v0', 100, 500]]
al_config = []
xi_config = []
for i in range(4):
    for j in range(4):
...
def parse_args():
    ...
    # Checkpointing
    parser.add_argument("--exp-name", type=str, default='train_xi', help="name of the experiment")
...
if __name__ == '__main__':
    for i in range(len(env_config)):
        for j in range(len(al_config)):
            ...
            if arglist.soft_update == 'adaptive1':
                arglist.xi_1 = xi_config[j][0]
                arglist.xi_2 = xi_config[j][1]
